...
#Author: Lance Yan
#Date: 2013-04-14
#Description: Convert xpath string in the all the files in <pages> folder to WebElement object.
#######################################################
...
&goThroughXpathFolder( $xpathdir, $pagesdir );

sub goThroughXpathFolder {
	...
	opendir( TEMPDIR, $xpathdir ) or die "can't open it:$!";
	my @dir = readdir(TEMPDIR);
	foreach (@dir) {
		if ( $_ =~ /java$/ ) {
			my $pageName = substr( $_, 1, index( $_, ".java" ) - 1 );
			&xpath2element( $xpathdir, $pagesdir, $pageName );
		}
	}
}

sub xpath2element {
	my ($xpathdir, $pagesdir, $pageName) = @_;
	...
	my @element = (
		"indent", "public XWebElement", "string", "{\n",
