...
#Author: Lance Yan
#Date: 2013-04-14
#Description: Convert xpath string to WebElement object.
#######################################################
...
sub xpath2element {
	...
	my $pageName          = &getLastUpdatedXpathFile($xpathdir);
	...
	print "Are you sure you want to convert xpath in interface file: \n"
	  . "$fullXpathFileName \nto class file: \n$fullJavaFileName \n? \n"
	  . "Press Enter for yes, press any other key for No\n ";
	my $YesorNo = <stdin>;
	chomp($YesorNo);
	if ( $YesorNo ne "" ) {
		exit;
	...
	my @element = (
		...
		"public XWebElement",
		"string","{\n",
		"indent","\t// btnLogin = span#loginPage_LoginButton;\n",
			...
			unless( $name[0] =~ /^\/\//){
				if ( $name[3] =~ /^\"\/\// ) {
					if ( $line =~ /%TEXT%/ ) {
						...
						$element[5] = "\t//$locator\n";
						$element[9] = "\t_xelement.setLocator(getReplacedLocator($name[1], text));\n";
					} else {
						$element[2] = "$name[1]()";
						$element[5] = "\t//$locator\n";
...
sub getLastUpdatedXpathFile {
	...
	return substr( $lastupdatedfile, 1, index( $lastupdatedfile, ".java" ) - 1 )
